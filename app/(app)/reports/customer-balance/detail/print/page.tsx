...
export default async function CustomerBalanceDetailAllPrintPage({
  ...
  const { groups, grandTotal } = await getCustomerBalanceDetailAll(company.id, range);
  ...
  return (
    <main className="mx-auto max-w-[8.5in] bg-white p-6 text-neutral-900 print:p-0">
      ...
      <ReportHeader company={company} title="Customer Balance Detail Report" coverage={coverage} />
      {groups.length === 0 ? (

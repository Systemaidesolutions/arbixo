...
export function SawtClient({ tin, registeredName, locations }: { tin: string; registeredName: string; locations: Branch[] }) {
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    fetch(`/api/reports/bir/sawt?from=${range.from}&to=${range.to}&locationId=${locationId}`)
      .then((r) => r.json())
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `SAWT_${range.from}_to_${range.to}.csv`;
    a.click();
    ...
    <main className="mx-auto max-w-5xl p-4 sm:p-8">
      ...
      <div className="mt-4 flex flex-wrap items-end gap-3 rounded-lg border border-neutral-200 p-4 print:hidden">
        ...
        <div className="ml-auto flex gap-2">
          <a href={`/api/reports/bir/sawt/dat?from=${range.from}&to=${range.to}&locationId=${locationId}`} download className="rounded border border-neutral-300 px-3 py-1.5 text-sm text-neutral-700 hover:bg-neutral-50">
            Export BIR .DAT
          </a>
      ...
      {loading || !data ? (
        ...
        <p className="mt-6 text-sm text-neutral-400">No creditable-withholding income in this period.</p>
      ) : (

...
type Group = { vendorId: string; vendorName: string; rows: DetailRow[]; subtotal: number };

export function VendorBalanceDetailAllClient({ registeredName }: { registeredName: string }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch("/api/reports/vendor-balance?detail=all")
      .then(async (r) => {
  ...
  function exportAll() {
    ...
    const out: (string | number)[][] = [
      ["Vendor Balance Detail Report", registeredName, "All Dates"],
      [],
    ];
  ...
  return (
    <main className="mx-auto max-w-4xl p-4 sm:p-8">
      <a href="/reports/vendor-balance" className="text-sm text-brand-blue hover:underline">
        ‹ Back to summary
      </a>
      <div className="mt-2 flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-medium text-neutral-900">Vendor Balance Detail Report</h1>
          <p className="mt-1 text-sm text-neutral-500">{registeredName} — All Dates</p>
        </div>
        <div className="flex shrink-0 gap-2 print:hidden">
          <button onClick={() => window.open(`/reports/vendor-balance/detail/print?_embed=1`, "_blank")} disabled={!groups} className={`${field} text-neutral-700 hover:bg-neutral-50 disabled:opacity-40`}>
            Print
          </button>
      ...
      {loading ? (
        ...
        <div className="mt-6 space-y-6">
          {groups.map((g) => (
            <div key={g.vendorId} className="overflow-hidden rounded-lg border border-neutral-200">
              <div className="bg-neutral-100 px-3 py-2 text-sm font-medium text-neutral-900">{g.vendorName}</div>

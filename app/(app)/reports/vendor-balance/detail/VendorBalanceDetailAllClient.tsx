...
export function VendorBalanceDetailAllClient({
  registeredName,
  initialDateFrom = "",
  initialDateTo = "",
}: {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetch(`/api/reports/vendor-balance?${rangeParams({ detail: "all" })}`)
      .then(async (r) => {
  ...
  function exportAll() {
    ...
    const out: (string | number)[][] = [
      ["Vendor Balance Detail Report", registeredName, coverage],
      [],
    ];
    ...
    <main className="mx-auto max-w-4xl p-4 sm:p-8">
      ...
      <div className="mt-2 flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 gap-2 print:hidden">
          <button onClick={() => window.open(`/reports/vendor-balance/detail/print?${rangeParams({ _embed: "1" })}`, "_blank")} disabled={!groups} className={`${field} text-neutral-700 hover:bg-neutral-50 disabled:opacity-40`}>
            Print
          </button>

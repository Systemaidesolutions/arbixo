...
export default async function PurchaseDocsListPage() {
  ...
  const [docs, vendors] = await Promise.all([
    prisma.purchaseDoc.findMany({ where: { companyId: company.id }, orderBy: [{ createdAt: "desc" }], take: 100 }),
    prisma.vendor.findMany({ where: { companyId: company.id }, select: { id: true, registeredName: true, tradeName: true } }),
  ]);
  const vendorName = new Map(vendors.map((v) => [v.id, v.registeredName || v.tradeName]));
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500">
              <th className="px-3 py-2">Transaction No.</th><th className="px-3 py-2">Date</th><th className="px-3 py-2">Supplier</th><th className="px-3 py-2">Reference</th><th className="px-3 py-2 text-right">Amount</th><th className="px-3 py-2">Status</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-neutral-100">
            {docs.length === 0 && <tr><td colSpan={6} className="px-3 py-6 text-center text-neutral-400">No purchase documents yet. Click “New” to create one.</td></tr>}
            {docs.map((d) => (
              <tr key={d.id} className="hover:bg-neutral-50">
                <td className="px-3 py-1.5"><a href={`/transactions/purchase-on-account/${d.id}`} className="font-mono text-xs text-brand-blue hover:underline">{d.transactionNo}</a></td>
                <td className="px-3 py-1.5">{new Date(d.transactionDate).toISOString().slice(0, 10)}</td>
                <td className="px-3 py-1.5">{(d.vendorId && vendorName.get(d.vendorId)) || "—"}</td>

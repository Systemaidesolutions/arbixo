...
export default async function PurchaseDocPage({ params }: { params: { id: string } }) {
    ...
    return (
      <main className="mx-auto max-w-3xl p-4 sm:p-8">
        ...
        <div className="mt-4 rounded-lg border border-neutral-200 p-4 text-sm text-neutral-700">
          ...
          <div>Reference: {doc.referenceNo ?? "—"} · PO: {doc.purchaseOrderNo ?? "—"}</div>
        ...
        <div className="mt-4 overflow-x-auto rounded-lg border border-neutral-200">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-neutral-100">
              {doc.lines.map((l) => (
                <tr key={l.id}><td className="px-3 py-1.5 font-mono text-xs">{l.itemCode ?? "—"}</td><td className="px-3 py-1.5">{l.description}</td><td className="px-3 py-1.5 text-right font-mono">{Number(l.quantity)}</td><td className="px-3 py-1.5 text-right font-mono">{formatPeso(Number(l.unitCost))}</td><td className="px-3 py-1.5 text-right font-mono">{formatPeso(Number(l.amount))}</td><td className="px-3 py-1.5 text-right font-mono">{formatPeso(Number(l.vatAmount))}</td></tr>
              ))}
            </tbody>
  ...
  // Draft — editable.
  const data = await loadPurchaseFormData(company.id);
  const initial = {
    id: doc.id, transactionNo: doc.transactionNo, transactionDate: iso(doc.transactionDate)!, vendorId: doc.vendorId,
    supplierTin: doc.supplierTin, locationId: doc.locationId, terms: doc.terms, dueDate: iso(doc.dueDate),
    referenceNo: doc.referenceNo, purchaseOrderNo: doc.purchaseOrderNo, currency: doc.currency, remarks: doc.remarks,
    ...
    lines: doc.lines.map((l) => ({
      ...
      unitCost: Number(l.unitCost), discountPercent: Number(l.discountPercent),

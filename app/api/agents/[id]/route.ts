...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.code && body.code !== existing.code) {
    ...
    if (duplicate) {
      return NextResponse.json({ error: `Agent code "${body.code}" is already in use` }, { status: 409 });
    }
  }
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  const entryCount = await prisma.ledgerEntry.count({ where: { agentId: params.id } });
  if (entryCount > 0) {
    return NextResponse.json(
      { error: "This agent has posted ledger entries and can't be deleted. Deactivate it instead." },
      { status: 409 }
    );
  }

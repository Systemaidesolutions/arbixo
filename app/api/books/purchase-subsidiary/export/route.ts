...
export async function GET(request: NextRequest) {
  ...
  const heading = (text: string, font: Partial<ExcelJS.Font>) => {
    ...
    ws.mergeCells(r.number, 1, r.number, 16);
  ...
  heading("PURCHASE SUBSIDIARY JOURNAL", { bold: true, size: 14 });
  ...
  ws.mergeCells(r1, 6, r1, 7); // VAT Purchases (Goods)
  ws.mergeCells(r1, 8, r1, 9); // Non-VAT Purchases (Goods)
  ...
  for (const r of data.rows) {
    ...
    for (const c of numberCols) row.getCell(c).numFmt = NUM;
  }
  ...
  return new NextResponse(buffer, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="purchase-subsidiary-journal_${from}_to_${to}.xlsx"`,
    },
  });
}
